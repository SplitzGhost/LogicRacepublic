class_name RankBar
extends Control
## Current rank, MMR, and how much is still missing until the next rank.
...
func set_mmr(value: int, animate := true) -> void:
	...
	_tw.tween_method(_set_shown, shown_mmr, float(value), 1.05)
...
func _bar_rect() -> Rect2:
	return Rect2(Vector2(0.0, 52.0), Vector2(size.x, 20.0))
...
func _refresh() -> void:
	...
	if _fill == null:
		return
	var info := Ranks.rank_progress(int(round(shown_mmr)))
	var bar := _bar_rect()
...
func _draw() -> void:
	...
	var mmr_text := "%d MMR" % int(round(shown_mmr))
	var mmr_w := Palette.font_bold.get_string_size(mmr_text, HORIZONTAL_ALIGNMENT_LEFT, -1, 30).x
	draw_string(Palette.font_bold, Vector2(size.x - mmr_w, 28.0), mmr_text,
	...
	var note := "Top rank reached" if bool(info["is_max"]) \
			else "%d MMR to %s" % [int(info["to_next"]), String(info["next_name"])]
